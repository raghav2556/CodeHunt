...
function Node({ x, y, size, delay }) {
  return (
    <motion.div
      ...
      animate={{ opacity: [0, 0.5, 0], y: [0, -50, -90] }}
...
export default function AuthScreen({
  ...
  const sendOtp = async () => {
    ...
    if (authData.username.length > 30) {
      setAuthMessageType("error");
      setAuthMessage("Username cannot exceed 30 characters");
      return;
    }
    ...
    if (authData.password.length > 64) {
      setAuthMessageType("error");
      setAuthMessage("Password cannot exceed 64 characters");
      return;
    }
    ...
    try {
      ...
      setAuthMessageType(response.ok ? "success" : "error");
  ...
  const validateAndSubmit = () => {
    ...
    if (!isLogin && !otpVerified) {
      setAuthMessageType("error");
      setAuthMessage("Please verify your email first");
      return;
    }
    handleAuth();
  };
      ...
      >
        <div className="glass rounded-2xl p-8 shadow-[0_0_80px_rgba(0,0,0,0.6)]">
          ...
          <div className="text-center mb-6">
            <p className="font-hud text-[0.58rem] tracking-[0.25em] text-[var(--neon-40)] uppercase mb-1">
              ◆ ACCESS TERMINAL ◆
            </p>
            <p className="font-hud text-[0.52rem] tracking-[0.18em] text-[var(--text-muted)] uppercase">
          ...
          <div className="flex rounded-lg bg-[rgba(0,0,0,0.4)] border border-[var(--border)] p-1 mb-6">
            {["LOGIN", "REGISTER"].map((tab, i) => (
              <motion.button
                key={tab}
                onClick={() => { resetForm(); setIsLogin(i === 0); }}
                whileTap={{ scale: 0.97 }}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {!isLogin && (
                <div>
                  <label className="block text-[0.57rem] font-hud tracking-[0.2em] text-[var(--text-muted)] uppercase mb-1.5">
                    HANDLE
                  </label>
                  <input
                    type="text"
                    ...
                    <>
                      ...
                      <div className="flex gap-3">
                        <motion.button
                          ...
                          className={`flex-1 py-3 rounded-xl text-xs font-hud transition-all ${
                            otpVerified
                        ...
                        <motion.button
                          ...
                          whileHover={{ scale: resendTimer > 0 || otpVerified ? 1 : 1.02 }}
                          whileTap={{ scale: resendTimer > 0 || otpVerified ? 1 : 0.97 }}
                          className="px-4 rounded-xl border border-[rgba(0,212,255,0.25)] bg-[rgba(0,212,255,0.05)] text-[var(--cyan)] text-xs font-hud disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          {otpVerified ? "DONE" : resendTimer > 0 ? `${resendTimer}s` : "RESEND"}
                        </motion.button>
                      </div>
                    </>
                  )}
                ...
                {authMessage && (
                  ...
                  >
                    <p className={`text-xs font-mono tracking-wide ${
                      authMessageType === "success" ? "text-green-300" : "text-red-300"
                    }`}>
                      {authMessageType === "success" ? "✓" : "⚠"}{" "}
                      {authMessage}
                    </p>
              ...
              {isLogin && (
                <button
                  ...
                  className="mt-4 w-full text-xs text-[var(--cyan)] hover:text-white transition-colors"
                >
                  Forgot Passkey?
                </button>
              )}
              ...
              <div className="mt-5 flex items-center gap-2">
                ...
                <p className="text-[0.52rem] font-mono text-[var(--text-muted)] px-2 uppercase tracking-wider">
                  alternative access
                </p>
                <div className="flex-1 divider-subtle" />
              </div>
              ...
              <div className="mt-4 space-y-3">
                <motion.button
                  ...
                  className="w-full py-3 rounded-xl border border-[rgba(0,212,255,0.25)] bg-[rgba(0,212,255,0.05)] text-[var(--cyan)] font-hud text-[0.62rem] tracking-[0.18em] uppercase hover:bg-[rgba(0,212,255,0.08)] hover:border-[rgba(0,212,255,0.45)] transition-all"
                >
                  ◉ Google Access
                </motion.button>
                ...
                <motion.button
                  ...
                  className="w-full py-3 rounded-xl border border-[rgba(168,85,247,0.25)] bg-[rgba(168,85,247,0.05)] text-purple-300 font-hud text-[0.62rem] tracking-[0.18em] uppercase hover:bg-[rgba(168,85,247,0.08)] hover:border-[rgba(168,85,247,0.45)] transition-all"
                >
                  ◉ GitHub Access
                </motion.button>
              </div>
          ...
          <button
            onClick={() => { resetForm(); setIsLogin(!isLogin); }}
            className="w-full mt-3 py-2 text-[0.62rem] font-hud text-[var(--cyan)] hover:text-[var(--neon)] transition-colors tracking-widest uppercase"
          >
        ...
        <p className="text-center text-[0.52rem] font-mono text-[rgba(212,237,223,0.2)] mt-6 tracking-widest">
